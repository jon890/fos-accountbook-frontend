...
- 함수 시그니처에 `familyUuid`를 명시적 인자로 받을 것 (Action에서 전달)
- 반환 타입 명시 필수
...
export async function getSomething(familyUuid: string, params: Params): Promise<SomeResponse> {
  // 비즈니스 규칙 검증
  ...
  // GET — serverApiGet<T>(url): T 직접 반환 (data 자동 언래핑)
  return serverApiGet<SomeResponse>(`/families/${familyUuid}/something?${query}`);
}

export async function createSomething(familyUuid: string, data: CreateData): Promise<void> {
  const requestBody: SomeRequest = {
    // 데이터 변환 (JSON.stringify는 serverApiPost 내부에서 처리)
    date: data.date ? new Date(data.date).toISOString() : new Date().toISOString(),
    ...data,
  };
  // POST — serverApiPost<T>(url, body): body는 객체 그대로 전달
  await serverApiPost<void>(`/families/${familyUuid}/something`, requestBody);
}
```

### HTTP 메서드별 utility 사용 규칙

| 메서드 | 함수 | 비고 |
|--------|------|------|
| GET | `serverApiGet<T>(url)` | data 자동 언래핑 |
| POST | `serverApiPost<T>(url, body?)` | body는 객체 전달 (JSON.stringify 불필요) |
| PUT | `serverApiPut<T>(url, body?)` | |
| PATCH | `serverApiPatch<T>(url, body?)` | |
| DELETE | `serverApiDelete<T>(url)` | |
| 특수 옵션 필요 시 | `serverApiClient(url, opts)` | skipAuth 등 |

## Dependencies

### Internal

- `src/lib/server/api/` — HTTP 클라이언트 (`serverApiGet`, `serverApiPost`, `serverApiPut`, `serverApiDelete`)
