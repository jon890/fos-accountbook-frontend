...
| 레이어 | 책임 |
|--------|------|
| `actions/` | Controller — `"use server"`, 인증, Zod 검증, revalidatePath |
| `services/` | Business Logic — API 호출, 쿼리 빌딩, 데이터 변환 |
| `lib/server/api/` | Infrastructure — HTTP 클라이언트 |

## Subdirectories

| Directory     | Purpose                                                                                  |
| ------------- | ---------------------------------------------------------------------------------------- |
| `app/`        | Next.js App Router — 페이지, 레이아웃, API Routes (see `app/AGENTS.md`)                  |
| `actions/`    | Server Actions — Controller 역할만 담당 (see `actions/AGENTS.md`)                        |
| `services/`   | 비즈니스 로직 레이어 — API 호출, 쿼리 빌딩, 변환 (see `services/AGENTS.md`)              |
